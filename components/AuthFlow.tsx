...
const AuthFlow: React.FC<{ onComplete: (user: User) => void; existingUser?: User }> = ({ onComplete, existingUser }) => {
  const [authMode, setAuthMode] = useState<'login' | 'signup'>('login');
  const [step, setStep] = useState<'auth' | 'profile'>(existingUser ? 'profile' : 'auth');
  ...
  const handleGoogleSignIn = async () => {
    ...
    // Page redirects to Google from here — nothing more to do in this component.
  };
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    // Google sign-in users already have an account — just save their profile.
    if (existingUser) {
      const saved = await StorageService.saveProfile(existingUser.id, profile);
      if (!saved) {
        setError('Something went wrong saving your profile. Please try again.');
        setIsGenerating(false);
        return;
      }
      onComplete({ ...existingUser, profile });
      return;
    }
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 p-4 transition-colors duration-300">
        <div className="w-full max-w-md bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-xl border border-slate-100 dark:border-slate-700">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-3 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-slate-700 dark:text-slate-200 font-medium hover:bg-slate-50 dark:hover:bg-slate-600 transition-colors"
          >
